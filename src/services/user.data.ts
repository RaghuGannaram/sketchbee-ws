...
const updateUserRecord = catchAsyncDataError(async function (userId: string, updateData: any | { [key in "avatar" | "background"]: string }): Promise<IUser> {
...
const updateUserFollowRecord = catchAsyncDataError(async function (followerId: string, followeeId: string): Promise<string> {
	...
	if (!followeeDoc) throw new DataError(DataErrors.DB_RECORD_NOT_FOUND, "followed not found (Can't follow non-existent user).");
